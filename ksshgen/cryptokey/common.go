...
func encodePEMBlock(b *pem.Block, passphrase string) ([]byte, error) {
	if passphrase == "" {
		return pem.EncodeToMemory(b), nil
	}

	eb, err := x509.EncryptPEMBlock(rand.Reader, b.Type, b.Bytes, []byte(passphrase), x509.PEMCipherAES256)
	if err != nil {
		return nil, err
	}
